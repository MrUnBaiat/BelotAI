...
from belot.evaluation.match_eval import evaluate_matches
from belot.memory import Episode, make_minibatch
from belot.model import RecurrentMAPPOModel
from belot.perturbed_heuristic import perturbed_heuristic_action
...
# to 128.
# ---------------------------------------------------------------------------
# C11  REGIME-FILTERED ACTOR LOSS  (the v8 change)
# ---------------------------------------------------------------------------
# MEASURED (v4_13 / EXP-A2, epoch-2848 weights, 128 chunks per regime,
# episode-equalised, 3 independent data draws):
#
#   regime      unbiased |mu|^2    B_simple      aligned with `perturbed`
#   self          -4.79e-06 (neg)  inf           undefined -- no direction at all
#   random         9.18e-05         23.5k-35.4k  yes  [+0.10, +0.53]
#   perturbed      5.44e-05         34.6k-62.2k  --
#   heuristic      3.02e-05         29.7k-120k   yes  [+0.14, +0.49]
#   frozen_new     2.15e-05         93.2k        yes  [+0.04, +0.41]
#   frozen_ref     3.56e-07         2.0M         no   [-0.10, +0.31]
#
# `self` has a NEGATIVE unbiased |mu|^2: the gradient is indistinguishable from
# zero and has no definable direction. Training the actor on self-play episodes
# therefore contributes variance and no mean -- the quadratic dilution penalty
# B_eff = B_simple / f^2 (verified analytically and by simulation).
#
# So the actor trains ONLY on `perturbed` episodes; the critic trains on
# EVERYTHING (it is a regression at EV 0.62, not at a stationary point, and
# benefits from every state).
#
# DEVIATION FROM THE PRE-REGISTERED 3x INCLUSION RULE, recorded deliberately.
# That rule selects ['random', 'perturbed']. But mixing two only-partially-aligned
# regimes SHRINKS the mean while variance is unchanged, so B_eff RISES:
#     perturbed only   B_eff 62,164
#     + 35% random     B_eff 88,929   <- worse
#     random only      B_eff 35,412
# Verified by simulation (MC 59,227 / 92,008 / 34,961 vs analytic 62,164 / 88,929
# / 35,412). The rule assumed inclusion adds signal; measurement shows it dilutes
# the direction. Deviation signed off by the user.
ACTOR_REGIMES = ("script",)          # C11: `script` == the perturbed heuristic

# Script-dominant so the actor batch is large per unit of rollout. A little self
# and random remain: they cost nothing for the actor (filtered out) and give the
# critic state diversity.
OPP_SELF           = 0.05
OPP_RANDOM         = 0.05
OPP_FROZEN         = 0.00
OPP_SCRIPT         = 0.90    # perturbed heuristic -- the non-descendant opponent
            ...
            if done:
                for i in range(4):
                    ...
                    if is_learning(e, i) and len(active_ep[k]) > 0:
                        active_ep[k].regime = opp_kind[e]      # C11: tag at storage
...
def update(model, optimizer, episodes, device, entropy_coef=ENTROPY_START):
    ...
    # C11 trap: normalise advantages over the ACTOR SUBSET ONLY. Pooling would draw
    # the mean/std partly from episodes the actor never trains on. Safe for the
    # critic: `returns` were computed BEFORE normalisation and are untouched.
    def is_actor(ep):
        return getattr(ep, "regime", "script") in ACTOR_REGIMES

    actor_eps = [ep for ep in episodes if is_actor(ep)]
    if not actor_eps:
        return {}                                   # C11 trap: empty-subset guard
    all_adv = torch.cat([ep.advantages for ep in actor_eps])
    ...
    for _ in range(PPO_ITERS):
        ...
        for start in range(0, len(episodes), step_size):
            ...
            # C11 trap: TWO global denominators. The critic sees every timestep;
            # the actor and entropy see only ACTOR_REGIMES timesteps. Using one
            # denominator for both silently rescales the effective learning rate
            # of whichever loss is wrong. Both are computed over the WHOLE step, so
            # micro-batch accumulation stays exactly equivalent to one big backward
            # (each chunk divides by the global count, never its own).
            mb_valid = float(sum(len(ep) for ep in mb))
            actor_valid = float(sum(len(ep) for ep in mb if is_actor(ep)))
            if mb_valid == 0 or actor_valid == 0:
                continue
            optimizer.zero_grad()
            ...
            for ms in range(0, len(mb), MICROBATCH_EPISODES):
                ...
                # C11: (B,T) mask selecting only ACTOR_REGIMES episodes. Built here
                # from the episode list, so memory.py::make_minibatch is untouched.
                a_mask = pad_mask * torch.tensor(
                    [1.0 if is_actor(ep) else 0.0 for ep in chunk],
                    dtype=pad_mask.dtype, device=device).unsqueeze(1)
                ...
                # C11: actor + entropy use a_mask / actor_valid; critic uses the
                # full pad_mask / mb_valid.
                actor_loss = -(torch.min(surr1, surr2) * a_mask).sum() / actor_valid
                critic_loss = (F.mse_loss(values, b_ret, reduction='none')
                               * pad_mask).sum() / mb_valid
                entropy_loss = (entropies * a_mask).sum() / actor_valid
                ...
                with torch.no_grad():
                    ...
                    # C11 trap: KL and clip_frac on the ACTOR subset, else they are
                    # diluted by steps that contributed no policy gradient at all --
                    # which would also mislead the TARGET_KL early stop.
                    kl_s += float((((ratio - 1) - logratio) * a_mask).sum() / actor_valid)
                    cf_s += float(((torch.abs(ratio - 1.0) > CLIP_EPSILON).float()
                                   * a_mask).sum() / actor_valid)
                    bid_mask = (b_masks[..., 32:].sum(-1) > 0).float() * a_mask
                    play_mask = a_mask - bid_mask
