...
from belot.memory import make_minibatch
...
# The estimator needs several DISJOINT chunks of a fixed episode count. This was
# `MINIBATCH_EPISODES`, which was 128 when the tool was written. The trainer later
# moved to one optimizer step per rollout via gradient accumulation, setting
# `MINIBATCH_EPISODES = None` ("the whole rollout") and keeping 128 as the
# forward/backward chunk under the name `MICROBATCH_EPISODES`. Falling through to
# it preserves what the statistic means; taking the literal step size would give a
# single chunk and no estimate at all.
CHUNK = T.MINIBATCH_EPISODES or T.MICROBATCH_EPISODES
